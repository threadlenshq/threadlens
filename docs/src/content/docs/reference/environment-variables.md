...
| `SCOUT_AI_BRIDGE_URL` | Empty | Optional Docker host CLI bridge | Docker-facing bridge URL generated by `pnpm run docker:dev`, usually `http://host.docker.internal:4761`. |
| `SCOUT_AI_BRIDGE_TOKEN_FILE` | Empty | Optional Docker host CLI bridge | In-container token file path generated by Docker dev, usually `/run/secrets/scout-ai-bridge-token`. |
| `SCOUT_AI_BRIDGE_HOST_TOKEN_FILE` | Empty | Optional Docker host CLI bridge | Host token path used only by Docker Compose for the read-only token bind mount. |
| `SCOUT_AI_BRIDGE_DISABLE` | Empty | Optional Docker host CLI bridge opt-out | Set to `1` before `pnpm run docker:dev` to skip host bridge bootstrap. |
